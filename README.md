...
>Note from Cableboy1515: Hey all! I want to be clear and upfront that this is a vibe-coded election software project. I wanted to create beacuse an organization I participate in was wanting to hold electronic elections, but all platforms required paying for a service if you have over 20 or so voters. We have roughly 150, so I decided to see what I could create with my buddy, Claude. I tried to take careful consideration while building this platform with over 240 initial commits, but the reality is that I am not a coder or election security expert. I'm hopeful that this is a good start and the community can analyze, offer suggestionss and help this platform grow.
>
...
>
>Hopefully together we can make an excellent open-source project! Please use at own risk, no guarantees.
>
...
- **Vote tallies** — the script independently re-tabulates every question. For ranked-choice questions it runs the same IRV or STV algorithm (same tie-breaks, same Droop quota, same Gregory transfer) and prints the winner(s) and round-by-round counts — these must match the admin results page. For single/multiple-choice questions it shows weighted totals that must match the admin panel counts.
