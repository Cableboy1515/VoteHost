  ...
  case "$TUNNEL_CHOICE" in
    ...
    2)
      ...
      if [ "$_VH_ISOLATE" = "1" ]; then
        ...
        say ""
        say "  2. Open your tailnet policy file (no visual-editor shortcut for the"
        say "     isolated setup — the tag changes must be in the file):"
        say "       https://login.tailscale.com/admin/acls/file"
        say ""
        say "     The file is a single JSON object. tagOwners, nodeAttrs, and"
        say "     grants/acls are TOP-LEVEL keys — not nested inside each other."
        say "     Replace the entire file with this (for a grants-based tailnet):"
        say ""
        say "       {"
        say "         \"tagOwners\": {"
        ...
        say "       }"
        say ""
        say "     The grants entry replaces the default wildcard rule — other"
        say "     devices still reach each other; tag:votehost is excluded."
        say ""
        say "     ! Common mistake: do NOT paste tagOwners/nodeAttrs INSIDE the"
        say "       grants array. They must be at the same level as grants."
        say "       Error 'unknown field tagOwners'? That's what happened."
        say ""
        say "     Legacy 'acls' file (older tailnets)? grants and acls can't mix."
        say "     Use this instead of the grants block above:"
        say "       \"acls\": ["
        say "         { \"action\": \"accept\","
        say "           \"src\":    [\"autogroup:member\"],"
        say "           \"dst\":    [\"autogroup:member:*\"] }"
        say "       ]"
        say "     (tagOwners and nodeAttrs stay at the top level either way.)"
        say ""
        say "     Click Save when done."
        say ""
        ...
        say "       https://login.tailscale.com/admin/dns  ->  Enable HTTPS"
        say ""
        say "  2. Grant Funnel to your tailnet devices. Easiest path (one click):"
        say "       https://login.tailscale.com/admin/acls"
        say "       Expand the 'Funnel' section -> click 'Add Funnel to policy'"
        say ""
        say "     Or, edit the policy file directly:"
        say "       https://login.tailscale.com/admin/acls/file"
        say "       Add nodeAttrs as a TOP-LEVEL key (a sibling of grants/acls,"
        say "       NOT nested inside the array), then Save:"
        say "         \"nodeAttrs\": ["
    ...
    if TS_FQDN=$(_wait_for_ts_fqdn); then
      ...
      case "$_funnel_status" in
        *"not available"*|*"not enabled"*|*"requires"*|*"cannot"*|*"Error"*)
          ...
          warn "  • Funnel node attribute: open admin console -> Access controls,"
          warn "    expand Funnel, click 'Add Funnel to policy'. Or edit acls/file."
