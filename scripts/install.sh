  ...
  case "$TUNNEL_CHOICE" in
    ...
    2)
      ...
      if [ "$_VH_ISOLATE" = "1" ]; then
        ...
        say "     isolated setup — the tag + ACL changes must be in the file):"
        say "       https://login.tailscale.com/admin/acls/file"
        say ""
        say "     Your policy file is a single JSON object. Merge these three keys"
        say "     into it (create any that don't exist), so the file ends up roughly"
        say "     like this:"
        say ""
        say "       {"
        say "         \"tagOwners\": {"
        ...
        say "       }"
        say ""
        say "     The acls rule replaces the default {\"src\":[\"*\"], \"dst\":[\"*:*\"]}"
        say "     entry — your other devices still reach each other; tag:votehost is"
        say "     excluded from peer traffic. Click Save when done."
        say ""
        ...
        say "       https://login.tailscale.com/admin/acls/file"
        say "       Add this key to the top-level JSON object (create it if it"
        say "       doesn't exist yet) and click Save:"
        say "         \"nodeAttrs\": ["
