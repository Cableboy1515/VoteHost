  ...
  case "$TUNNEL_CHOICE" in
    ...
    2)
      ...
      if [ "$_VH_ISOLATE" = "1" ]; then
        ...
        say ""
        say "  2. Update your tailnet policy file:"
        say "       https://login.tailscale.com/admin/acls/file"
        say ""
        say "     ── New Tailscale user / untouched policy file? ─────────────────"
        say "     Replace the entire file with this and Save:"
        say ""
        say "       {"
        say "         \"tagOwners\": {"
        ...
        say "       }"
        say ""
        say "     ── Existing tailnet (you have custom groups, SSH rules, etc.)? ─"
        say "     Add these as TOP-LEVEL keys alongside your existing grants/acls"
        say "     (NOT nested inside the array), and replace the default wildcard"
        say "     {\"src\":[\"*\"],\"dst\":[\"*\"],\"ip\":[\"*\"]} grant with the member-only"
        say "     entry shown above."
        say ""
        say "       \"tagOwners\": { \"tag:votehost\": [\"autogroup:admin\"] },"
        say "       \"nodeAttrs\": [{ \"target\": [\"tag:votehost\"], \"attr\": [\"funnel\"] }]"
        say ""
        say "     Legacy 'acls' tailnet? grants and acls can't mix in one file."
        say "     Use this for the access rule instead of the grants entry:"
        say "       \"acls\": ["
        say "         { \"action\": \"accept\","
        say "           \"src\":    [\"autogroup:member\"],"
        say "           \"dst\":    [\"autogroup:member:*\"] }"
        ...
        say "       https://login.tailscale.com/admin/acls/file"
        say ""
        say "     New Tailscale user? Replace the whole file with:"
        say "       { \"nodeAttrs\": [{ \"target\": [\"autogroup:member\"], \"attr\": [\"funnel\"] }] }"
        say ""
        say "     Existing tailnet? Add nodeAttrs as a TOP-LEVEL key (sibling of"
        say "     grants/acls, NOT nested inside the array), then Save:"
        say "       \"nodeAttrs\": ["
