...
const WORDMARK_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 10 98.03 24" width="196" height="48" role="img" aria-label="VoteHost Elections">
...
function makeDoc(title: string): PDFKit.PDFDocument {
  return new PDFDocument({
    ...
    margin: 50,
    bufferPages: true,
    info: { Title: `${title} — Results`, Creator: "VoteHost" },
  })
}
...
function renderContent(doc: PDFKit.PDFDocument, data: ExportData, s: Spacing): void {
...
function applyPageDecorations(doc: PDFKit.PDFDocument, generatedOn: string): void {
  const range = doc.bufferedPageRange()
  for (let i = 0; i < range.count; i++) {
    doc.switchToPage(range.start + i)
    doc.fontSize(8).fillColor(MUTED).font("Helvetica").text(
      `Generated by VoteHost Elections · ${generatedOn} · Page ${i + 1} of ${range.count}`,
      50, 730, { align: "center", width: 495, lineBreak: false }
    )
  }
}
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  renderContent(doc1, data, NORMAL)
  ...
  if (range1.count === 2 && finalY1 < 200) {
    ...
    renderContent(doc2, data, COMPACT)
  ...
  applyPageDecorations(chosen, generatedOn)
